...
import { View, Text, Pressable, ScrollView, SafeAreaView, Alert, TextInput } from 'react-native';
...
export default function CreateGameScreen() {
  ...
  const handleCustomConfirm = () => {
    ...
    if (isNaN(val) || val < MIN_DURATION || val > MAX_DURATION) {
      Alert.alert('Invalid Duration', `Please enter a duration between ${MIN_DURATION} and ${MAX_DURATION} minutes.`);
      return;
    }
    setDuration(val);
    setCustomMode(false);
  ...
  const effectiveDuration = customMode ? (parseInt(customInput, 10) || duration) : duration;
          ...
          <View style={{ flexDirection: 'row', gap: 10 }}>
            {DURATIONS.map((d) => (
              <Pressable
                key={d}
                style={{ flex: 1 }}
                onPress={() => { setDuration(d); setCustomMode(false); setCustomInput(''); }}
              >
                <View
                  style={{
                    paddingVertical: 12,
                    borderRadius: 12,
                    alignItems: 'center',
                    backgroundColor: !customMode && duration === d ? Colors.blue : Colors.bg.secondary,
                    borderWidth: 1,
                    borderColor: !customMode && duration === d ? Colors.blue : 'rgba(255,255,255,0.07)',
                  }}
                >
                  <Text
                    style={{
                      color: !customMode && duration === d ? '#fff' : Colors.text.secondary,
                      fontSize: 14,
            ...
            >
              <View
                style={{
                  ...
                  borderRadius: 12,
                  alignItems: 'center',
                  backgroundColor: customMode ? Colors.blue : Colors.bg.secondary,
                  borderWidth: 1,
                  borderColor: customMode ? Colors.blue : 'rgba(255,255,255,0.07)',
                }}
              >
                <Ionicons name="pencil" size={14} color={customMode ? '#fff' : Colors.text.secondary} />
              </View>
            </Pressable>
          </View>
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10, marginTop: 4 }}>
              ...
              >
                <TextInput
                  ...
                  style={{ flex: 1, color: Colors.text.primary, fontSize: 15, fontFamily: 'Poppins_700Bold' }}
                />
                <Text style={{ color: Colors.text.muted, fontSize: 13, fontFamily: 'Poppins_400Regular' }}>min</Text>
              </View>
              <Pressable onPress={handleCustomConfirm}>
                ...
                >
                  <Text style={{ color: '#fff', fontSize: 13, fontFamily: 'Poppins_700Bold' }}>Set</Text>
                </View>
              </Pressable>
            </View>
          )}
        ...
        >
          ...
          {[
            ...
            <View key={text} style={{ flexDirection: 'row', gap: 12, alignItems: 'center' }}>
              ...
              <Text style={{ color: Colors.text.secondary, fontSize: 13, fontFamily: 'Poppins_400Regular', flex: 1 }}>
                {text}
              </Text>
            </View>
          ))}
          ...
          <Animated.View style={btnStyle}>
            ...
            >
              <Text style={{ color: '#fff', fontSize: 16, fontFamily: 'Poppins_700Bold' }}>
                {loading ? 'Creating...' : `Create ${customMode ? (parseInt(customInput,10) || duration) : duration}min Room 🎲`}
              </Text>
            </LinearGradient>
          </Animated.View>
        </Pressable>
